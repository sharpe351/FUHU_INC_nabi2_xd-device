...
#  BUILD=kot49h
#fi # klp-dev
ROOTDEVICE=android_device_fuhu_nabi2_xd
DEVICE=nabi2_xd
MANUFACTURER=FUHU_INC

for COMPANY in nabi2_xd broadcom elan invensense nvidia nxp widevine
do
  ...
  nabi2_xd)
    TO_EXTRACT="\
            ...
            system/lib/hw/camera.tegra.so \
            system/lib/hw/sensors.nabi2_xd.so \
            system/lib/hw/lights.nabi2_xd.so \
            system/vendor/lib/drm/libdrmwvmplugin.so \
            system/vendor/libwvm.so \
            "
    ;;
  broadcom)
    TO_EXTRACT="\
  ...
  nvidia)
    TO_EXTRACT="\
            ...
            system/etc/nvram_4334.txt \
	    system/etc/nvram_4334r1.txt \
	    ...
	    system/lib/libnvmmlite_msaudio.so \
